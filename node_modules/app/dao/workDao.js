...
workDao.saveWork = function (uid,id,name,code,cb) {
	var sql = 'update user_works set wname = ?,codes = ?,updateTime = ? where id = ?';
	var time = Math.floor(new Date().getTime()/1000);
	var args = [name,code,time,id];
	pomelo.app.get('dbclient').query(sql,args,function(err, res) {
		if(err){
			utils.invokeCallback(cb, err, null);
		}else{
			utils.invokeCallback(cb, err, {"id":id,"uid":uid,"codes":code,"wname":name,"updateTime":time});
		}
	});
};
...
workDao.addAmount = function (id, amount, cb) {
	var sql = 'update user_works set amount = amount + 1 where id = ?';
	var args = [id];
	pomelo.app.get('dbclient').query(sql,args,function(err, res) {
		if(err){
			utils.invokeCallback(cb, err, null);
...
workDao.publishPage = function (uid,code,name,desText, cb) {
	...
	async.series([
		...
		function(callback){
			sql = 'insert into system_user_pages (uid,wname,nickName,codes,desText,updateTime) values(?,?,?,?,?,?)';
			var time = Math.floor(new Date().getTime()/1000);
			args = [uid,name,nickName,code,desText,time];
			pomelo.app.get('dbclient').query(sql,args,function(err, res) {
				if(err){
					data = null;
				}else{
					data = {"id":res.insertId,"uid":uid,"codes":code,"wname":name,"des-text":desText,"updateTime":time};
				}
			});
		}
	],function(err,results) {
